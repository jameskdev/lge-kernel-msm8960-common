...
{
	int ret, i;
	int level, level_max, level_min;

	struct timed_vibrator_data *vib = NULL;
	...
	level_max = vib->vibe_data->low_max;
	level_min = vib->vibe_data->low_min;

	if (vib->vibe_data->amp >= GAIN_MAX)
		level = level_max;
	else if (vib->vibe_data->amp < GAIN_MIN)
		level = 0;
	else
		level = (((level_max - level_min) * vib->vibe_data->amp)/ GAIN_MAX) + level_min;
	...
	printk("[%s] Vibrator read error on pmic %d \n", __func__, ret);
	if (ret < 0) {
