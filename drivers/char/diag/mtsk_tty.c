...
int mtsk_tty_modem_request(const unsigned char *buf, int count)
{
	...
	memcpy(driver->usb_buf_mdm_out, (char *)buf + 6,
...
static void mtsk_tty_close(struct tty_struct *tty, struct file *file)
{
	...
	case MTS_TTY_FACTORY_MODE:
		printk(KERN_INFO "mtsk_tty_close FAIL."
				 "why MTS_TTY_OPEN is not opend  %d,%d \n",
				 mts_state, 0);
		break;
	default :
		return;
...
static int mtsk_tty_ioctl(struct tty_struct *tty, unsigned int cmd,
	...
	case MTSK_TTY_CHK_REG:
		...
		if (copy_to_user((void *)arg, (const void *)&info,
				 sizeof(unsigned int)) == 0) {
			printk(KERN_INFO "%s MTSK_TTY_CHK_REG: "
					 "complete sending pif cable info %d\n",
					 __func__, info);
		} else {
			printk(KERN_INFO "%s MTSK_TTY_CHK_REG: "
					 "fail to sending pif cable info\n",
					 __func__);
		}
		break;
#ifdef CONFIG_MTS_USE_HSIC
	case MTSK_TTY_WRITE_ENABLE:
		info = (short)(!(driver->in_busy_hsic_write) &&
		       driver->hsic_ch && driver->hsic_device_enabled &&
		       mts_state != MTS_TTY_SEND_MASK);
		if (copy_to_user((void *)arg, (const void *)&info, sizeof(info))
		    != 0) {
			printk(KERN_INFO "%s MTSK_TTY_WRITE_ENABLE "
					 "fail to sending hsic ch info\n",
					 __func__);
		}
		break;
	case MTSK_TTY_CHK_HSIC:
		...
		if (copy_to_user((void *)arg, (const void *)&info,
				 sizeof(info)) != 0) {
			printk(KERN_INFO "%s MTSK_TTY_CHK_HSIC "
					 "fail to sending hsic enabled \n",
					 __func__);
		}
		break;
	case MTSK_TTY_SET_56K :
		...
		if (copy_to_user((void *)arg, (const void *)&info,
				 sizeof(info)) != 0) {
			printk(KERN_INFO "%s MTSK_TTY_STATE_DATA "
					 "fail to sending hsic ch info\n",
					 __func__);
		}
		break;
...
static int mtsk_tty_write(struct tty_struct *tty, const unsigned char *buf,
			  int count) {
	int result;
	struct mts_tty *mtsk_tty_drv = NULL;
