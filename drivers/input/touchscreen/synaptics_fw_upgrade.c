...
static int synaptics_fw_upgrade_debug_mask;
...
{
	return (unsigned long)((unsigned long)SynaImage[0] +
...
int FirmwareUpgrade(struct synaptics_ts_data *ts)
{
	...
	if (unlikely(ts->fw_path[0] != 0)) {
		...
		fd = sys_open((const char __user *) ts->fw_path, O_RDONLY, 0);
		if (fd < 0) {
		...
		ret = sys_newstat(ts->fw_path, (struct stat *)&fw_bin_stat);
		if (ret < 0) {
